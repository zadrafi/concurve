...
| ~~`%>%` → `|>`~~ | **done on `release/3.0.5`** | 5 in `R/` (one plotly chain in `R/plot.likelihood_function.R`) plus 6 in `vignettes/supported.Rmd`. `@importFrom dplyr %>%` went with the first set, `magrittr` (Suggests) with the second. `dplyr` stays in Imports (14 `dplyr::` calls) |
