...
```{r echo=TRUE, fig.height=4.5, fig.width=6}
fecurve <- curve_meta(fe, cores = 4L)
```
...
```{r echo=TRUE, fig.height=4.5, fig.width=6}
recurve <- curve_meta(re, cores = 4L)
```
...
l <- concurve::curve_meta(res, method = "uni", robust = TRUE, cluster = dat$trial, adjust = TRUE, steps = 1000, cores = 4L)
...
a <- curve_meta(x = res, measure = "default", method = "mv", parm = "factor(x)1", cores = 4L)
