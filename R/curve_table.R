curve_table <- function(data, levels, type = "data.frame") {
  ...
  subdf <- mclapply(levels, FUN = function(i) (subset(data, intrvl.level == i)))
  subdf <- data.frame(do.call(rbind, subdf))
  subdf$intrvl.level <- (subdf$intrvl.level * 100)
  subcolnames <- c("Lower Limit", "Upper Limit", "Limit Ratio", "Interval Level (%)", "P-value", "S-value (bits)")
...
utils::globalVariables(c("subdf", "Lower Limit", "Limit Ratio", "Upper Limit", "Interval Level", "P-value", "S-value"))
