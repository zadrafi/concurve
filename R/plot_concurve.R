...
polygon(c(max(data$lower.limit), data$lower.limit),
        c(min(data$pvalue), data$pvalue), col = "#239a9880", border = NA)
...
text(x = 1.27, y = 3, paste("MLE:",
                             round(((max(data$lower.limit) + min(data$upper.limit)) / 2), 3)),
     cex = 0.93, col = "black")

text(x = 1.27, y = 10, paste("50% CI:",
                             round(unname((quantile(data$lower.limit, prob = 0.50))), 3), "-",
                             round(unname((quantile(data$upper.limit, prob = 0.50))), 3),  sep=" ", collapse=", "),
     cex = 0.93, col = "black")

text(x = 1.27, y = 17, paste("75% CI:",
                             round(unname((quantile(data$lower.limit, prob = 0.25))), 3), "-",
                             round(unname((quantile(data$upper.limit, prob = 0.75))), 3),  sep=" ", collapse=", "),
     cex = 0.93, col = "black")

text(x = 1.27, y = 24, paste("95% CI:",
                             round(unname((quantile(data$lower.limit, prob = 0.05))), 3), "-",
                             round(unname((quantile(data$upper.limit, prob = 0.95))), 3),  sep=" ", collapse=", "),
     cex = 0.93, col = "black")

text(x = 1.27, y = 31, paste("99% CI:",
                             round(unname((quantile(data$lower.limit, prob = 0.01))), 3), "-",
                             round(unname((quantile(data$upper.limit, prob = 0.99))), 3),  sep=" ", collapse=", "),
     cex = 0.93, col = "black")
}
