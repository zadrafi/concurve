plotsint<-function(x, measure="default",
  ...
  ggplot(data = x) +
    geom_point(aes(x = lower.limit, y = svalue), color = color, fill = color, alpha = 0.5, shape = 21, stroke = 0.8, size = 1) +
    geom_point(aes(x = upper.limit, y = svalue), color = color, fill = color, alpha = 0.5, shape = 21, stroke = 0.8, size = 1) +
    geom_ribbon(aes(x = lower.limit, ymin = max(svalue), ymax = svalue), fill = color, alpha = 0.30) +
    geom_ribbon(aes(x = upper.limit, ymin = max(svalue), ymax = svalue), fill = color, alpha = 0.30) +
    labs(title = title,
         subtitle = subtitle,
         caption = caption,
         x = xaxis,
   ...
   {if(measure == "ratio") scale_x_log10(breaks = scales::pretty_breaks(n = 10))} +
    ...
    if(measure == "default") {
      annotate("segment", x = 0, xend = 0, y = 0, yend = 13.28,
               colour = "#990000", alpha = 0.5, size = 1)
    } else if(measure == "ratio") {
      annotate("segment", x = 1, xend = 1, y = 0, yend = 13.28,
               colour = "#990000", alpha = 0.5, size = 1)
    }
}
